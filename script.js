...
// Ovoz fayllarini yuklash: audio papkasidan ovozlarni o‘ynatish uchun
const audioEat = new Audio('audio/eat.mp3')
const audioBonus = new Audio('audio/bonus.mp3')
const audioWallHit = new Audio('audio/wallHit.mp3')
const audioCrash = new Audio('audio/crash.mp3')
...
const translations = {
	en: {
		...
		statsText:
			'Total Time: {time}s | Total Score: {score} | Avg Level: {level}',
		gameOverScore: 'Score: ',
	...
	ru: {
		...
		instructions:
			'Смахивайте или используйте стрелки на клавиатуре для управления змейкой',
		statsText:
			'Общее время: {time}с | Общий счёт: {score} | Средний уровень: {level}',
		gameOverScore: 'Очки: ',
	...
	uz: {
		...
		instructions:
			'Ilonni boshqarish uchun suring yoki klaviaturadagi o‘q tugmalarini bosing',
		statsText:
			'Umumiy vaqt: {time}s | Umumiy ball: {score} | O‘rtacha bosqich: {level}',
		gameOverScore: 'Ball: ',
		...
		wallActions: {
			none: 'Amal tanlang',
			addWall: 'To‘siq qo‘yish',
			removeWall: 'To‘siq olib tashlash',
		},
	},
	tr: {
		...
		speedLevelText: 'Hız: ',
		instructions:
			'Yılanı kontrol etmek için kaydırın veya klavyedeki yön tuşlarını kullanın',
		statsText:
			'Toplam Süre: {time}s | Toplam Puan: {score} | Ortalama Seviye: {level}',
		gameOverScore: 'Puan: ',
...
let highScore = localStorage.getItem('highScore')
	...
	: 0 // Eng yuqori ball, localStorage dan olinadi
...
function updateLanguage() {
	...
	pauseBtn.innerHTML = isPaused
		? `<i class="fas fa-play"></i> ${t.resumeBtn}`
		: `<i class="fas fa-pause"></i> ${t.pauseBtn}`
...
let highScores = localStorage.getItem('highScores')
	...
	: [] // Eng yaxshi ballar ro‘yxati
let totalLevels = localStorage.getItem('totalLevels')
	? parseInt(localStorage.getItem('totalLevels'))
	: 0 // Umumiy bosqichlar
...
let bonusSpawnTime = null // Bonus paydo bo‘lgan vaqt
...
const bonusTypes = [
	{
		type: 'slow',
		color: '#00BFFF',
		glowColor: 'rgba(0, 191, 255, 0.8)', // Ko‘k akslanuvchi
		effect: () => {
			gameSpeed = baseGameSpeed * 1.5
		},
		duration: 5000,
	}, // Sekinlashtiruvchi bonus
	{
		type: 'double',
		color: '#FFD700',
		glowColor: 'rgba(255, 215, 0, 0.8)', // Sariq akslanuvchi
		effect: () => {
			bonusEffect = 'double'
		},
		duration: 5000,
	}, // Ikki baravar ball beruvchi bonus
]
...
function resizeCanvas() {
	...
	const aspectRatio = 560 / 550 // Yangi nisbat (560/550)
...
function startGame() {
	...
	food = {
		x: Math.floor(tileCount * 0.75),
		y: Math.floor(tileCount * 0.75),
	...
	walls = []
	generateWalls()
	dx = 0 // Boshlang‘ich yo‘nalish yo‘q
	dy = 0 // Boshlang‘ich yo‘nalish yo‘q
	score = 0
	level = 1
	speedLevel = 1
	baseGameSpeed = 160 // Sekinroq tezlik
...
function gameUpdate() {
	...
	// Ilon harakati faqat yo‘nalish bo‘lsa amalga oshiriladi
	if (dx !== 0 || dy !== 0) {
		...
		if (gameMode === 'endless' || wallMode === 'open') {
			...
			audioCrash.play() // Devorga urilish ovozi
			gameOver()
			return
		}
		...
		if (
			...
			audioCrash.play() // O‘ziga urilish ovozi
			gameOver()
			return
		}
		...
		if (walls.some(wall => wall.x === head.x && wall.y === head.y)) {
			audioWallHit.play() // To‘siqqa urilish ovozi
			gameOver()
			return
		}
		...
		if (head.x === food.x && head.y === food.y) {
			audioEat.play() // Oddiy ovqat yeyish ovozi
		...
		if (bonus && head.x === bonus.x && head.y === bonus.y) {
			audioBonus.play() // Bonus yeyish ovozi
			const currentBonus = bonusTypes.find(b => b.type === bonus.type)
			const remainingTime = Math.max(
				0,
				Math.floor((bonusDuration - (Date.now() - bonusSpawnTime)) / 1000)
			)
			score += remainingTime
	...
	if (bonus && bonusSpawnTime && Date.now() - bonusSpawnTime >= bonusDuration) {
		bonus = null
		bonusSpawnTime = null
	}

	// Bonus faol bo‘lsa, qolgan vaqtni yangilash
...
function drawGame() {
	...
	if (bonus) {
		...
		// Bonusning animatsiyali qismini chizish
		ctx.save()
		ctx.translate(bonusX + gridSize / 2, bonusY + gridSize / 2)

		// Pulsatsiya effekti uchun o‘lcham
		...
		// Qadoqlovchi halqa
		ctx.beginPath()
		ctx.arc(0, 0, (gridSize / 2.5) * 1.2, 0, Math.PI * 2)
		ctx.strokeStyle = currentBonus.glowColor.replace('0.8', '0.4')
		ctx.lineWidth = 1.5
		ctx.stroke()

		// Bonusning asosiy shari
		ctx.beginPath()
		...
		bonusGradient.addColorStop(0.5, currentBonus.color)
		bonusGradient.addColorStop(
			1,
			currentBonus.color.replace(/[^,]+(?=\))/, '0.6')
		)
		...
		ctx.lineWidth = 1
		ctx.stroke()
		ctx.shadowColor = currentBonus.glowColor
		ctx.shadowBlur = 25
		ctx.fill()
		ctx.shadowColor = 'transparent'
		ctx.shadowBlur = 0
		...
		// Bonus qolgan vaqtini animatsiyasiz chizish
		if (bonusSpawnTime) {
			ctx.save()
			ctx.translate(bonusX + gridSize / 2, bonusY + gridSize / 2)
			const remainingTime = Math.max(
			...
			ctx.font = `bold ${gridSize}px Roboto`
			ctx.fillStyle = '#ffffff'
			ctx.textAlign = 'center'
			ctx.textBaseline = 'middle'
			ctx.fillText(remainingTime, 0, gridSize / 2 + 10)
			ctx.restore()
		}
	}
